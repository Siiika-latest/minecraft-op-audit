...
AIGC:
  ...
  Label: '1'
  ProduceID: '8bd534f0-68fb-4ada-918f-a03ac74208b2'
  PropagateID: '8bd534f0-68fb-4ada-918f-a03ac74208b2'
  ReservedCode1: 'aa4a6164-7b2e-4ffc-9a69-58dfb6ec7e2e'
  ReservedCode2: 'aa4a6164-7b2e-4ffc-9a69-58dfb6ec7e2e'
---
...
| 视图 | 说明 |
|---|---|
| **排行榜** | 按天出七张榜：**管理员行为榜**、**死亡次数榜**、**力工破坏方块榜**、**力工放置方块榜**、**力工天榜**（破坏+放置之和）、**击杀生物榜**、**内鬼榜**（玩家互杀）。每行显示名次、玩家、数值与百分位徽章（配色参考 FF14 Logs），可切换统计日期 |
...
> **任何视图里的玩家名都可以点击**，直接筛选该玩家的全部记录（含他作为凶手的击杀记录，
> 标记「他是凶手」）；不想筛了用筛选栏右侧的「清除玩家筛选」。
...
| **死亡次数榜** | 当天该玩家**死亡**的次数 |
| **力工破坏方块榜 / 力工放置方块榜** | 当天该玩家 stats 汇总事件的破坏 / 放置方块数之和 |
| **力工天榜** | 当天该玩家破坏 + 放置方块数之和（力工总榜） |
| **击杀生物榜** | 当天该玩家击杀生物数（stats 汇总之和） |
| **内鬼榜** | 当天该玩家作为击杀者出现的玩家死亡次数（实时，不与 stats 双计） |

查某个玩家时，他作为**凶手**的击杀记录会一并显示（含时间、被杀者、死因），
标记为「他是凶手」；死者视角标记为「他死亡」。

### 分数怎么算（v1.2.2：色阶最低分制）

分值取**所在色阶的最低分**，按名次优先铺满 7 色：

| 名次 | 分值 | 色阶（含义） |
|---|---|---|
| 1 | `100` | 金色（当日第一） |
| 2 | `99` | 粉色（仅次于第一） |
| 3 | `91` | 橙色（顶尖，区间 91–98 的最低分） |
| 4 | `76` | 紫色（优秀，区间 76–90 的最低分） |
| 5 | `51` | 蓝色（中上，区间 51–75 的最低分） |
| 6 | `26` | 绿色（中下，区间 26–50 的最低分） |
| 7 及以后 | `1` | 灰色（垫底，区间 1–25 的最低分） |

因此：

- **7 个互不相同的数值正好一人一色**
- **并列同名次同分同色**，下一名照常推进（dense rank）：两人并列第 1 都是 `100` 金，
  下一个不同值仍是第 2 名 `99` 粉 —— 第二名永远是 99，也让 7 色尽可能都有人
- 少于 7 个不同值时从金色起依次使用；超过 7 个不同值后新增名次均为灰色 `1`
...
色阶函数 `tier_of()` 仍按完整区间判断（`100` 金 / `99` 粉 / `91–98` 橙 / `76–90` 紫 /
`51–75` 蓝 / `26–50` 绿 / `1–25` 灰），因此接口字段 `pct` 可以填区间内任意值，
新计分规则只是固定取各档最低分。

配色沿用 FF14 Logs 的观感（深色主题下直接用原味配色；浅色主题下换成同色系深色版，保证白底可读）。

> 这个分数是**队内相对排名**的色阶标识，不是绝对评价 —— 人少的时候排到 100 只是因为当天只有你在动。跨天比没意义，同一天横着比才有意义。
...
挂机零行为的玩家不产生任何输出。**内鬼（玩家互杀）统计**不依赖汇总：死亡事件带 `kp` 标记
（击杀者是玩家时为 1），看板据此从既有死亡数据实时推导，旧数据按「击杀者名
在已知玩家集合中」兑底判断，僵尸这生物名不会混进内鬼榜。查某个玩家时会连带显示他作为凶手的击杀记录（时间、被杀者、死因）。
